...
export default function CreatePaymentDebit({ code, payments, onCreate }) {
	...
	const onBankChange = async (value) => {
		...
		const data = await get_bank_branches(value);
	...
	const onFinish = async (values) => {
		try {
			...
			const post_data = {
				...
				bankAccountData: {
					values: [
						...payments.bankAccountData.values.map((el) => {
							return {
								number: el.number,
								cbu: el.cbu,
		...
		<Inner>
			...
			>
				...
				>
					...
					>
						<Select allowClear={true} onChange={onBankChange}>
							{banks.map((bank) => (
				...
				<FloatingLabel
					label={t("account-number-lnl")}
					value={formValues?.debit_accountNumber}
					hint={t("account-number-lnl")}
				>
					<Form.Item
